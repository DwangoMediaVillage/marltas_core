"""Helper function to run CNN-DQN actor."""
import logging
import os
...
from dqn.cnn.replay_buffer import ReplayBufferClient
from dqn.param_distributor import ParamDistributorClient
from dqn.policy import PolicyParam


def run_actor(init_policy_param: PolicyParam, process_index: int, start_queue: Queue, config: CNNConfigBase) -> None:
    """Run CNN-DQN actor.
            ...
            if step % config.send_sample_interval == 0:
                if len(local_buffer):
                    replay_buffer_client.append_sample(
                        SampleFromActor.concat(samples=[local_buffer.popleft() for _ in range(len(local_buffer))],
