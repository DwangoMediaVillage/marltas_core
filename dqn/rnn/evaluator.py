"""Implementation of evaluator for RNN-DQN."""
import logging
import time
...
class Evaluator(EvaluatorBase):
    ...
    def evaluate(self, request: EvaluationRequest) -> EvaluationResult:
        ...
        for _ in range(self.n_eval):
            ...
            state = [self.policy.model.get_init_state()]  # reset hidden state of RNN
            self.policy.on_partial_reset(0)
            while not done:
                prediction, _, state = self.policy.infer([obs], state)
