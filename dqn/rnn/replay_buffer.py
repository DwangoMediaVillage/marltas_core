"""Implementation of replay buffer for RNN-DQN."""
import logging
...
from dqn.rnn.datum import (Loss, SampleFromActor, SampleFromBuffer,
...
class ReplayBuffer(ReplayBufferBase):
    ...
    def split_sample(self, sample: SampleFromActor) -> Tuple[List[SampleFromActor], np.ndarray]:
        """Split sample from actor"""
        return split_sample_from_actor(sample, np_defs=self.sample_from_actor_def), sample.loss

    def post_process_sample(self, samples: List[SampleFromActor], weight: np.ndarray) -> Any:
        s = SampleFromBuffer.from_buffer_samples(samples=samples, sample_from_actor_def=self.sample_from_actor_def)
        s.weight = weight
        return s
...
class ReplayBufferServicer(ReplayBufferServerServicerBase):
    ...
    def bytes_to_sample(self, bytes_data: bytes) -> SampleFromActor:
        return SampleFromActor.from_bytes(bytes_data=bytes_data, np_defs=self.sample_from_actor_def)
...
class ReplayBufferServer:
    """Helper class for runnig replay buffer on main thread."""
