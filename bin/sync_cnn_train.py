...
from dqn.cnn.actor import Actor
from dqn.cnn.config import CNNConfigBase
from dqn.cnn.datum import Batch, SampleFromActor
from dqn.cnn.evaluator import Evaluator
from dqn.cnn.learner import Learner
from dqn.cnn.replay_buffer import ReplayBuffer
...
class Config(CNNConfigBase):
    """Configuration of CNN synchronized training."""
...
def main(log_dir: Path, config: Config, logger: logging.Logger = logging.getLogger(__name__)) -> None:
    ...
    actor = Actor(config=config, init_policy_param=policy_param)
    ...
    try:
        sync_train(log_dir=log_dir,
                   ...
                   evaluator=evaluator,
                   actor=actor,
                   concat_samples_from_actor=partial(SampleFromActor.concat,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Synchronized CNN-DQN training.")
